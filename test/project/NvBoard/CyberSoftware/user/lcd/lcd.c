...
static const DVTiming DVTTfg = h640_v480_r60;
...
/* 线程控制块定义 */
struct rt_thread th_colorbar;
/* 定义线程栈 */
rt_uint8_t rt_thcolorbar_stack[512];

/* colorbar 线程 入口函数 */
void th_colorbar_en(void *p_arg)
{
...
/* show colorbar */
long show_colorbar(void)
{
    /* 初始化线程 */
    rt_thread_init(&th_colorbar,                /* 线程控制块 */
                   "th_colorbar ",              /* 线程名称，唯一 */
                   th_colorbar_en,              /* 线程入口地址 */
                   RT_NULL,                     /* 线程形参 */
                   &rt_thcolorbar_stack[0],     /* 线程栈起始地址 */
                   sizeof(rt_thcolorbar_stack), /* 线程栈大小 */
                   1,                           /* 线程优先级 */
                   1);                          /* 线程时间片 */
    /* 启动线程 */
    rt_thread_startup(&th_colorbar);

    printf("start to show colorbar\r\n");
}
// MSH_CMD_EXPORT(show_colorbar, show colorbar);
