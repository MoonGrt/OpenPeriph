...
//   val OAR1 = Reg16Init() // 自身地址寄存器1
//   val OAR2 = Reg16Init() // 自身地址寄存器2
//   val DR = Reg16Init()   // 数据寄存器
//   val SR1 = Reg16Init()  // 状态寄存器1
//   val SR2 = Reg16Init()  // 状态寄存器2
//   val CCR = Reg16Init()  // 时钟控制寄存器
//   val TRISE = Reg16Init() // TRISE寄存器
...
//   val DUALF = SR2(7)     // 双标志
//   val PEC = SR2(8 downto 15) // 数据包错误检查
...
//     is(3) { // 接收数据
//       when(clockTick) {
//         rxShiftReg := rxShiftReg(6 downto 0) ## io.i2c.sda
//         bitCounter := bitCounter + 1
//         when(bitCounter === 7) {
//           // 将接收到的数据放入RX FIFO
...
// // I2C主接口定义
// case class I2cMaster() extends Bundle with IMasterSlave {
//   val scl = Bool()
//   val sda = Bool()

//   override def asMaster(): Unit = {
//     out(scl)
//     inout(sda) // I2C是开漏输出，需要双向
//   }
// }
