...
object Axi4RamPort {
  def apply(config: Axi4Config, ram: Mem[Bits]) = {
    val port = new Axi4RamPort(config)
    port.attach(ram)
    port
  }
...
case class Axi4RamPort(config: Axi4Config) extends ImplicitArea[Axi4Shared] {
  ...
  val logic = Handle {
    new Area {
      ...
      Vec(readAddrStream, writeAddrStream) <> StreamDemux(arw, U(arw.write), 2 )
      ...
      val readData = ram.readSync(address = address).resized
      ...
      axi.arw.ready.noBackendCombMerge
    }
  }
...
object Axi4RamMultiPort {
  def apply(portCount: Int, dataWidth: Int, byteCount: BigInt, idWidth: Int) = {
    ...
    new Axi4RamMultiPort(axiConfig, wordCount, portCount)
  }
  def apply(config: Axi4Config, wordCount: BigInt, portCount: Int) =
    new Axi4RamMultiPort(config, wordCount, portCount)
}

class Axi4RamMultiPort(config: Axi4Config, wordCount: BigInt, portCount: Int)
    extends Component {
  val io = new Bundle {
  ...
  if (config.useLock || config.useRegion || config.useCache || config.useProt || config.useQos)
    SpinalWarning(
      "Axi4RamMultiPort might not support Axi4 Lock, Region, Cahce, Prot and Qos featrues!"
    )
