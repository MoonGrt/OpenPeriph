...
class Axi4RamMultiPort(config: Axi4Config, wordCount: BigInt, portCount: Int) extends Component {
    val io = new Bundle {
    ...
    if (config.useLock || config.useRegion || config.useCache || config.useProt || config.useQos)
        SpinalWarning("Axi4RamMultiPort might not support Axi4 Lock, Region, Cahce, Prot and Qos featrues!")
