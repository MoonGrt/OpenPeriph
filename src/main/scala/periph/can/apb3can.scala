...
//   val MCR = Reg32Init()   // 主控制寄存器
//   val MSR = Reg32Init()   // 主状态寄存器
//   val TSR = Reg32Init()   // 发送状态寄存器
//   val RF0R = Reg32Init()  // 接收FIFO0寄存器
//   val RF1R = Reg32Init()  // 接收FIFO1寄存器
//   val IER = Reg32Init()   // 中断使能寄存器
//   val ESR = Reg32Init()   // 错误状态寄存器
//   val BTR = Reg32Init()   // 位时序寄存器
//   val TI0R = Reg32Init()  // 发送邮箱标识符寄存器0
//   val TDT0R = Reg32Init() // 发送邮箱数据长度控制寄存器0
//   val TDL0R = Reg32Init() // 发送邮箱数据寄存器0低
//   val TDH0R = Reg32Init() // 发送邮箱数据寄存器0高
//   val TI1R = Reg32Init()  // 发送邮箱标识符寄存器1
//   val TDT1R = Reg32Init() // 发送邮箱数据长度控制寄存器1
//   val TDL1R = Reg32Init() // 发送邮箱数据寄存器1低
//   val TDH1R = Reg32Init() // 发送邮箱数据寄存器1高
//   val TI2R = Reg32Init()  // 发送邮箱标识符寄存器2
//   val TDT2R = Reg32Init() // 发送邮箱数据长度控制寄存器2
//   val TDL2R = Reg32Init() // 发送邮箱数据寄存器2低
//   val TDH2R = Reg32Init() // 发送邮箱数据寄存器2高
//   val RI0R = Reg32Init()  // 接收邮箱标识符寄存器0
//   val RDT0R = Reg32Init() // 接收邮箱数据长度控制寄存器0
//   val RDL0R = Reg32Init() // 接收邮箱数据寄存器0低
//   val RDH0R = Reg32Init() // 接收邮箱数据寄存器0高
//   val RI1R = Reg32Init()  // 接收邮箱标识符寄存器1
//   val RDT1R = Reg32Init() // 接收邮箱数据长度控制寄存器1
//   val RDL1R = Reg32Init() // 接收邮箱数据寄存器1低
//   val RDH1R = Reg32Init() // 接收邮箱数据寄存器1高
//   val FMR = Reg32Init()   // 过滤器主寄存器
//   val FM1R = Reg32Init()  // 过滤器模式寄存器
//   val FS1R = Reg32Init()  // 过滤器尺度寄存器
//   val FFA1R = Reg32Init() // 过滤器分配寄存器
//   val FA1R = Reg32Init()  // 过滤器激活寄存器
//   val F0R1 = Reg32Init()  // 过滤器组0寄存器1
//   val F0R2 = Reg32Init()  // 过滤器组0寄存器2
//   val F1R1 = Reg32Init()  // 过滤器组1寄存器1
//   val F1R2 = Reg32Init()  // 过滤器组1寄存器2
...
//   val txShiftReg = Reg(Bits(128 bits)) init(0) // CAN帧最大长度
//   val rxShiftReg = Reg(Bits(128 bits)) init(0)
...
//       is(2) { // 发送标识符
//         when(clockTick) {
//           io.can.tx := txShiftReg(127 - bitCounter)
//           bitCounter := bitCounter + 1
...
//       is(1) { // 接收标识符
//         when(clockTick) {
//           rxShiftReg := rxShiftReg(126 downto 0) ## io.can.rx
//           bitCounter := bitCounter + 1
//           when(bitCounter === 11) { // 11位标识符
//             rxState := 2
//           }
...
// // CAN主接口定义
// case class CanMaster() extends Bundle with IMasterSlave {
//   val tx = Bool()
//   val rx = Bool()
