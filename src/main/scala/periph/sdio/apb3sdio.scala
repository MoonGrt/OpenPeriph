...
// SDIO主接口定义
case class Sdio() extends Bundle with IMasterSlave {
  val clk = Bool()
  val cmd = Analog(Bool())
...
case class Apb3Sdio(
  ...
  val cmdCounter = Reg(UInt(7 bits)) init(0)
  val dataCounter = Reg(UInt(32 bits)) init(0)
  val cmdShiftReg = Reg(Bits(128 bits)) init(0)
  ...
  val cmdMachine = new Area {
    switch(cmdState) {
      ...
      is(1) { // 发送命令
        when(clockTick) {
          io.sdio.cmd := cmdShiftReg(127)
          cmdShiftReg := cmdShiftReg(126 downto 0) ## B"0"
      ...
      is(2) { // 等待响应
        when(clockTick) {
          when(WAITRESP =/= 0) {
            // 接收响应
            cmdShiftReg := cmdShiftReg(126 downto 0) ## io.sdio.cmd
            cmdCounter := cmdCounter + 1
            when(cmdCounter === 127) { // 长响应
              cmdState := 3
  ...
  ctrl.onRead(0x44) {
    when(RXDAVL) {
      ...
      io.apb.PRDATA := rxFifo.io.pop.payload
    }
  }
