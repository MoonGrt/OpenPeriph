...
class Axi4Riscv(
    coreConfig: RiscvCoreConfig,
    iCacheConfig: InstructionCacheConfig,
    dCacheConfig: DataCacheConfig,
    debug: Boolean,
    interruptCount: Int
) extends Component {
  ...
  val io = new Bundle {
    ...
    val interrupt = if (interruptCount != 0) in(Bits(interruptCount bit)) else null
    val debugResetIn = if (debug) in Bool () else null
    val debugResetOut = if (debug) out Bool () else null
    val debugBus = if (debug) slave(Apb3(Axi4Riscv.getDebugApbConfig())) else null
  }

  if (interruptCount != 0)
    coreConfig.add(
      new SimpleInterruptExtension(exceptionVector = 0x20)
        .addIrq(id = 4, pins = io.interrupt, IrqUsage(isException = false), name = "io_interrupt")
    )
  val nativeInstructionBusExtension = if (!iCached) coreConfig.add(new NativeInstructionBusExtension) else null
  val cachedInstructionBusExtension =
    if (iCached) coreConfig.add(new CachedInstructionBusExtension(iCacheConfig, false, true)) else null
...
object Axi4Riscv {
  ...
  //   val report = SpinalConfig(onlyStdLogicVectorAtTopLevelIo = true, targetDirectory = "rtl").generateVerilog({
  ...
  //     new Axi4Riscv(coreConfig, iCacheConfig, dCacheConfig, debug, interruptCount)
  //   })
  // }
}
