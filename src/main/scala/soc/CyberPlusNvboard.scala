...
case class CyberPlusNvboardConfig(
...
object CyberPlusNvboardConfig {
  def default = {
    val config = CyberPlusNvboardConfig(
...
class CyberPlusNvboard(config: CyberPlusNvboardConfig) extends Component {
  // Legacy constructor
  def this(axiFrequency: HertzNumber) {
    this(CyberPlusNvboardConfig.default.copy(axiFrequency = axiFrequency))
  }
  ...
  io.seg := axi.gpioCtrl.io.gpio.write(31 downto 24)
  io.rgbled := axi.gpioCtrl.io.gpio.write(23 downto 18)
  axi.gpioCtrl.io.gpio.read(17) := io.uart_rx
  io.uart_tx := axi.gpioCtrl.io.gpio.write(16)
  axi.gpioCtrl.io.gpio.read(15 downto 8) := io.sw
  io.led := axi.gpioCtrl.io.gpio.write(7 downto 0)

  axi.gpioCtrl.io.gpio.read(31 downto 18) := B(0, 14 bits) // 保留空位
  axi.gpioCtrl.io.gpio.read(16) := False
  axi.gpioCtrl.io.gpio.read(7 downto 0) := B(0, 8 bits)
}
...
/* ---------------------------------- Demo Gen --------------------------------- */
/* ----------------------------------------------------------------------------- */
// CyberPlusNvboard-SoC with memory init
object CyberPlusNvboard {
  def main(args: Array[String]) {
    val config =
      SpinalConfig(verbose = true, targetDirectory = "rtl").dumpWave()
    val report = config.generateVerilog(
      InOutWrapper(
        new CyberPlusNvboard(
          CyberPlusNvboardConfig.default.copy(
